...
Provide a rigorous technical diagnosis in valid JSON with these exact keys:
{
  ...
  "recommendedParts": [
    { "sku": "K-XXXXX", "name": "Kohler Genuine Part Name", "quantity": 1, "priceInr": 1450 }
  ],
...
Note: State all replacement parts pricing in Indian Rupees (₹ INR).`;
...
function generateDeterministicDiagnosis(incident: any, device: any) {
  ...
  if (type === 'CONTINUOUS_LEAK') {
    ...
    if (flow > 10) {
      return {
        ...
        recommendedParts: [
          { sku: "K-1067341", name: "Kohler Commercial Flushometer Solenoid Valve Assembly", quantity: 1, priceInr: 3850 },
          { sku: "K-1032402", name: "High-Efficiency Diaphragm Rebuild Kit (1.28 GPF)", quantity: 1, priceInr: 1450 }
        ],
        estimatedRepairTimeMinutes: 25,
    ...
    return {
      ...
      recommendedParts: [
        { sku: "K-1032402", name: "Kohler Dual-Filtered EPDM Diaphragm Kit", quantity: 1, priceInr: 1450 },
        { sku: "K-4567-01", name: "Viton O-Ring & Vacuum Breaker Gasket Kit", quantity: 1, priceInr: 450 }
      ],
    ...
    return {
      ...
      recommendedParts: [
        { sku: "CL-501", name: "Hospital-Grade Quaternary Disinfectant Solution", quantity: 1, priceInr: 850 },
        { sku: "PP-204", name: "Kohler Touchless Paper Towel Roll Refill", quantity: 4, priceInr: 650 }
      ],
    ...
    return {
      ...
      recommendedParts: [
        { sku: "K-89010", name: "Kohler Industrial 6V CR-P2 Lithium Battery Pack", quantity: 1, priceInr: 950 },
        { sku: "K-3341-LENS", name: "Tripoint IR Optical Protective Window Kit", quantity: 1, priceInr: 520 }
      ],
  ...
  return {
    ...
    recommendedParts: [
      { sku: "K-10673-GEN", name: "Kohler Commercial Sensor Diagnostic Tool", quantity: 1, priceInr: 4200 }
    ],
...
function generateDeterministicChatReply(message: string, context: any) {
  ...
  if (q.includes('leak') || q.includes('water waste') || q.includes('wastage')) {
    ...
    return `Currently, the Kohler Sense telemetry engine is tracking ${wasted.toFixed(1)} L/min of water flow across active fixtures. ` +
      `Our sliding-window leak detection pipeline integrates volumetric loss in real time. For uncontained leaks, Kohler Sense triggers automated P1 dispatch with genuine Kohler OEM repair SKUs (K-1067341 [₹3,850] / K-1032402 [₹1,450]) to contain water loss within the target < 90-second SLA.`;
  }
